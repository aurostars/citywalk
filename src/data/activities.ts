import type { Activity } from "../types/domain";
import { assetPath } from "../lib/assetPath";

export const activities: Activity[] = [
  {
    id: "798-art-weekend",
    title: "798 当代艺术周末",
    type: "exhibition",
    price: 80,
    district: "朝阳区",
    venue: "798 艺术区",
    indoor: true,
    suitablePartySizes: ["solo", "pair", "group"],
    weatherKinds: ["sunny", "cloudy", "rain"],
    availableWeekdays: ["saturday", "sunday"],
    startTime: "10:00",
    schedule: "周六至周日 10:00-18:00",
    durationMinutes: 180,
    imageUrl: assetPath("images/activities/798-art-weekend.webp"),
    imageAlt: "798 艺术空间内参观展览的年轻人",
    summary: "从工业建筑里的当代展览开始，留出时间逛独立书店和园区小店。",
    editorOrder: 1,
  },
  {
    id: "gulou-weekend-market",
    title: "鼓楼胡同周末市集",
    type: "market",
    price: 45,
    district: "东城区",
    venue: "鼓楼东大街",
    indoor: false,
    suitablePartySizes: ["pair", "group"],
    weatherKinds: ["sunny", "cloudy"],
    availableWeekdays: ["saturday"],
    startTime: "13:00",
    schedule: "周六 13:00-18:00",
    durationMinutes: 150,
    imageUrl: assetPath("images/activities/gulou-weekend-market.webp"),
    imageAlt: "从鼓楼俯瞰北京胡同屋顶",
    summary: "沿鼓楼东大街慢慢逛手作摊位，再转进胡同寻找唱片和旧书。",
    editorOrder: 2,
  },
  {
    id: "liangma-river-live",
    title: "亮马河水岸音乐现场",
    type: "show",
    price: 0,
    district: "朝阳区",
    venue: "亮马河国际风情水岸",
    indoor: false,
    suitablePartySizes: ["pair", "group"],
    weatherKinds: ["sunny", "cloudy"],
    availableWeekdays: ["saturday"],
    startTime: "18:30",
    schedule: "周六 18:30-20:30",
    durationMinutes: 120,
    imageUrl: assetPath("images/activities/liangma-river-live.webp"),
    imageAlt: "亮马河两岸绿地与城市建筑",
    summary: "傍晚从河岸步道走到水边舞台，在城市灯光亮起时听一场轻松演出。",
    editorOrder: 3,
  },
  {
    id: "ncpa-weekend-concert",
    title: "国家大剧院周末音乐会",
    type: "show",
    price: 180,
    district: "西城区",
    venue: "国家大剧院音乐厅",
    indoor: true,
    suitablePartySizes: ["solo", "pair", "group"],
    weatherKinds: ["sunny", "cloudy", "rain"],
    availableWeekdays: ["sunday"],
    startTime: "14:30",
    schedule: "周日 14:30-16:30",
    durationMinutes: 120,
    imageUrl: assetPath("images/activities/ncpa-weekend-concert.webp"),
    imageAlt: "白天的国家大剧院椭圆形建筑外景",
    summary: "在室内音乐厅安排一段完整的午后，适合想安静听完一场演出的周末。",
    editorOrder: 4,
  },
  {
    id: "panjiayuan-vintage-market",
    title: "潘家园旧物早市",
    type: "market",
    price: 0,
    district: "朝阳区",
    venue: "潘家园旧货市场",
    indoor: false,
    suitablePartySizes: ["solo", "pair", "group"],
    weatherKinds: ["sunny", "cloudy"],
    availableWeekdays: ["saturday"],
    startTime: "08:00",
    schedule: "周六 08:00-12:00",
    durationMinutes: 180,
    imageUrl: assetPath(
      "images/activities/panjiayuan-vintage-market.webp",
    ),
    imageAlt: "潘家园市场摆放旧物与老灯具的摊位",
    summary: "早点到旧货市场，从旧书、器物和海报摊位里慢慢淘一件有故事的小物。",
    editorOrder: 5,
  },
  {
    id: "shougang-industrial-exhibition",
    title: "首钢园工业遗址特展",
    type: "exhibition",
    price: 120,
    district: "石景山区",
    venue: "首钢园三高炉",
    indoor: true,
    suitablePartySizes: ["pair", "group"],
    weatherKinds: ["sunny", "cloudy", "rain"],
    availableWeekdays: ["saturday", "sunday"],
    startTime: "10:00",
    schedule: "周六至周日 10:00-17:00",
    durationMinutes: 150,
    imageUrl: assetPath(
      "images/activities/shougang-industrial-exhibition.webp",
    ),
    imageAlt: "蓝天下的首钢园三高炉工业建筑",
    summary: "在高炉工业空间里看城市更新主题展，顺路观察保留下来的钢铁结构。",
    editorOrder: 6,
  },
  {
    id: "xiangshan-morning-hike",
    title: "香山清晨登高线",
    type: "hike",
    price: 10,
    district: "海淀区",
    venue: "香山公园",
    indoor: false,
    suitablePartySizes: ["solo", "pair", "group"],
    weatherKinds: ["sunny", "cloudy"],
    availableWeekdays: ["sunday"],
    startTime: "07:30",
    schedule: "周日 07:30-11:30",
    durationMinutes: 240,
    imageUrl: assetPath("images/activities/xiangshan-morning-hike.webp"),
    imageAlt: "香山山顶俯瞰北京西郊景色",
    summary: "趁游客增多前走一段林间上升路线，在观景点休息后原路返回。",
    editorOrder: 7,
  },
  {
    id: "wenyu-river-cycling",
    title: "温榆河公园骑行",
    type: "hike",
    price: 0,
    district: "朝阳区",
    venue: "温榆河公园朝阳示范区",
    indoor: false,
    suitablePartySizes: ["pair", "group"],
    weatherKinds: ["sunny", "cloudy"],
    availableWeekdays: ["saturday"],
    startTime: "09:00",
    schedule: "周六 09:00-12:30",
    durationMinutes: 210,
    imageUrl: assetPath("images/activities/wenyu-river-cycling.webp"),
    imageAlt: "温榆河支流水岸与湿地植被",
    summary: "沿湿地绿道完成一段平缓骑行，在开阔草地和水岸驿站之间停留。",
    editorOrder: 8,
  },
  {
    id: "universal-beijing-day",
    title: "北京环球度假区一日游",
    type: "entertainment",
    price: 528,
    district: "通州区",
    venue: "北京环球度假区",
    indoor: false,
    suitablePartySizes: ["solo", "pair", "group"],
    weatherKinds: ["sunny", "cloudy"],
    availableWeekdays: ["saturday", "sunday"],
    startTime: "09:30",
    schedule: "周六至周日 09:30-19:30",
    durationMinutes: 600,
    imageUrl: assetPath("images/activities/universal-beijing-day.webp"),
    imageAlt: "北京环球度假区过山车与主题园区",
    summary: "把整天留给主题园区，按体力安排项目、演出和休息时段。",
    editorOrder: 9,
  },
  {
    id: "tianqiao-musical-night",
    title: "天桥艺术中心音乐剧之夜",
    type: "show",
    price: 480,
    district: "西城区",
    venue: "北京天桥艺术中心",
    indoor: true,
    suitablePartySizes: ["solo", "pair", "group"],
    weatherKinds: ["sunny", "cloudy", "rain"],
    availableWeekdays: ["sunday"],
    startTime: "19:30",
    schedule: "周日 19:30-22:00",
    durationMinutes: 150,
    imageUrl: assetPath("images/activities/tianqiao-musical-night.webp"),
    imageAlt: "北京天桥艺术中心观众席与舞台",
    summary: "把周日晚间留给剧场，提前到场，并为演出结束后的返程预留时间。",
    editorOrder: 10,
  },
  {
    id: "indoor-ski-weekend",
    title: "室内滑雪周末场",
    type: "entertainment",
    price: 420,
    district: "顺义区",
    venue: "室内滑雪场",
    indoor: true,
    suitablePartySizes: ["pair", "group"],
    weatherKinds: ["sunny", "cloudy", "rain"],
    availableWeekdays: ["saturday"],
    startTime: "13:00",
    schedule: "周六 13:00-17:00",
    durationMinutes: 240,
    imageUrl: assetPath("images/activities/indoor-ski-weekend.webp"),
    imageAlt: "室内滑雪场的宽阔雪道",
    summary: "用一个下午体验室内雪道，在练习与休息之间安排适合自己的节奏。",
    editorOrder: 11,
  },
  {
    id: "immersive-theatre-weekend",
    title: "朝阳沉浸式戏剧周末场",
    type: "entertainment",
    price: 380,
    district: "朝阳区",
    venue: "郎园 Station",
    indoor: true,
    suitablePartySizes: ["pair", "group"],
    weatherKinds: ["sunny", "cloudy", "rain"],
    availableWeekdays: ["saturday"],
    startTime: "19:30",
    schedule: "周六 19:30-22:00",
    durationMinutes: 150,
    imageUrl: assetPath(
      "images/activities/immersive-theatre-weekend.webp",
    ),
    imageAlt: "沉浸式艺术装置中的光影与观众",
    summary: "在室内行走式空间中跟随叙事推进，给周六夜晚留出完整体验时间。",
    editorOrder: 12,
  },
];
